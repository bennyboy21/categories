...
var myName = urlParams.get('name')
var gameID = urlParams.get('gameID')

if(myName != undefined && gameID == undefined) 
{
    ...
    firebase.database().ref("TDSB/Categories/Waiting").once('value', function(snapshot) 
    {
        ...
        {
            firebase.database().ref("TDSB/Categories/Waiting/" + gameID).set(
            {
                "Name": myName,
                "Other_Player":false,
                "doneWaiting":false,
            ...
            firebase.database().ref("TDSB/Categories/Games/" + gameID).set(
            {
                ...
                "round_num":0,
                "players_turn":title(myName),
                "player_one":title(myName)
            }
            )

            findingOpponent()

            window.location = "game.html?name=" + name + "&gameID=" + gameID
        }
...
{
    function loaded() {
        firebase.database().ref("TDSB/Categories/Games/" + gameID + "/").on("value", function() 
        {
            console.log(player_one)
            if(snapshot.val().player_one == title(myName))
            {
                player1 = true
                alert("Player 1")
            }
        ...
        // firebase.database().ref("TDSB/Categories/Games/" + gameID + "/").on("value", function(snapshot) 
        // {
        //     document.getElementById("your-Points-Count").innerText = snapshot.val()
        // })

        if(player1 == true)
        {
            firebase.database().ref("TDSB/Categories/Games/" + gameID + "/").on("child_changed", function(snapshot) 
            {
                if(opponentName == "")
                {
                    roundNum = round_num
    ...
    if(player1 && roundNum < 5 || !player1 && roundNum <= 5)
    {
        ...
        if(!noText)
        {
            ...
            if(noErrors)
            {
                ...
                {        
                    firebase.database().ref("TDSB/Categories/Games/" + gameID + "/").update(
                    {
                ...
                {    
                    ...
                    if(roundNum == 1)
                    {
                        firebase.database().ref("TDSB/Categories/Games/" + gameID + "/" + title(myName) + "'s Answers/").set({
                            "1": answers[0],
                        })
                        answers = []
                    }
                    ...
                    {
                        firebase.database().ref("TDSB/Categories/Games/" + gameID + "/" + title(myName) + "'s Answers/").update({
                            "2": answers[0],
                        })
                        answers = []
                    }
...
function youLose() 
{
    ...
    if(player1)
    {
        firebase.database().ref("TDSB/Categories/Games/" + gameID).remove()
    }
